(async function () {
  ...
  costs.forEach((yearlyCostBreakdown) => {
    ...
    const yearlyTotalOperatingExpense = (
      yearlyTotalCosts + yearlyTotalRevenue
    ).toFixed(2);
    ...
    const datasets = Object.entries(byCategoryMapping)
      .map((keyValue, index) => {
    ...
    new Chart(document.getElementById(`${year}-costs`), {
      type: "bar",
      data: {
...
function byCategory(costs) {
  ...
  costs.forEach((c) => {
    var categoryEntries =
      c.category in byCategoryMapping ? byCategoryMapping[c.category] : [];
...
function totalCosts(expenditures) {
  return parseFloat(
    expenditures
      .filter((item, _a, _b) => item.cost > 0)
      .reduce((total, cost) => total + (cost.cost || 0), 0)
      .toFixed(2)
  );
}
